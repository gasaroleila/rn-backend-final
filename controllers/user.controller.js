...
export const pyUploadFile = async (req, res) => {
  ...
  try {
    ...
    return res.json({
      message: "failed to upload property files",
      status: 500,
    });
  }
};
...
export const registerUser = async (req, res) => {
  try {
    ...
    user.accountType = req.body.accountType ? "voter" : "admin"
...
export const loginWithLink = async (req, res) => {
  try {
    ...
    if (user.loginLink && user.loginLink.code != req.params.userCode) {
      return res.status(400).send("Invalid Code!");
    }

    let time = new Date();
    let creationTimeInSeconds = user.loginLink.creationTime.getTime() / 1000;
    ...
    if (!user.requestPasswordReset) {
      ...
      return res
        ...
        .send({
          status: 401,
          success: false,
          message: "You did not request password resetting",
          data: user,
        });
    } else {
  ...
  try {
    ...
    if (!user)
      return res
        ...
        .send({
          status: "error",
          message: "Unable to find the user with the provided email",
        });
...
export const checkCode = async (code, userId) => {
  try {
    ...
    if (user.loginLink && user.loginLink.code != code) {
      return res.status(400).send("Invalid Code!");
    }
    let time = new Date();
    let creationTimeInSeconds = user.loginLink.creationTime.getTime() / 1000;
